...
class Estimate extends BaseController
{
    ...
    public function save()
    {
        ...
        $workType = $this->request->getPost('work_type');
        $discount = floatval($this->request->getPost('discount') ?? 0);
        $transportationCost = floatval($this->request->getPost('transportation_cost') ?? 0);

        // Item arrays
        $description = $this->request->getPost('description');
        $materialCost = $this->request->getPost('material_cost');
        $labourHour = $this->request->getPost('labour_hour');
        $gpPercentage = $this->request->getPost('gp_percentage');
        $sellingPrice = $this->request->getPost('selling_price');
        $difference = $this->request->getPost('difference');
        $quantity = $this->request->getPost('quantity');
        $total = $this->request->getPost('total');

        // Basic validation
        if (empty($customerId) || empty($address) || empty($workType)) {
            return $this->response->setJSON([
                'status' => 'error',
                'message' => 'Customer, Address and Work Type are required.'
            ]);
        }
        ...
        $grandTotal = bcadd(
            bcsub($subtotal, (string) $discount, 4),
        ...
        foreach ($description as $key => $desc) {
            ...
            $itemsArray[] = [
                ...
                'material_cost' => number_format($materialCost[$key] ?? 0, 4, '.', ''),
                'labour_hour' => number_format($labourHour[$key] ?? 0, 2, '.', ''),
                'gp_percentage' => number_format($gpPercentage[$key] ?? 0, 2, '.', ''),
                'selling_price' => number_format($sellingPrice[$key] ?? 0, 4, '.', ''),
                'difference' => number_format($difference[$key] ?? 0, 2, '.', ''),
                'quantity' => number_format($qty, 4, '.', ''),
                'total' => number_format($total[$key] ?? 0, 4, '.', ''),
                'item_order' => $key + 1
            ];
        }
        ...
        if ($estimateId) {
            ...
            $msg = 'Estimate Generated Successfully.';
        }
    ...
    public function edit($id)
    {
        ...
        $items = $estimateItemModel
            ->select('item_id, estimate_id, description,material_cost, selling_price, quantity, total, item_order')
        ...
        $data = [
            'estimate' => $estimate,
            'items' => $items,
            'customers' => $customerModel->where('status', 9)->orderBy('customer_id', 'DESC')->findAll(),
            'customer' => $customer
        ];
