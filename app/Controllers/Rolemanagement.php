...
class Rolemanagement extends Controller
{
    ...
    public function store()
    {
        ...
        // Insert Permissions
        $menuIds = array_keys($access_data);

        $this->roleModel->update($role_id, [
            'role_permissions' => json_encode($menuIds)
        ]);
...
// }
        ...
        foreach ($totalRec as $role) {
            $permissionIds = json_decode($role->role_permissions, true) ?? [];

            $permissions = $this->roleMenuModel
                ->whereIn('rolemenu_id', $permissionIds)
                ->where('status', 1)
                ->findAll();
    ...
    public function update($id)
    {
        ...
        $newPermissions = array_keys($access_data);

        // Existing role
        $existingRole = $this->roleModel->find($id);
        $oldPermissions = json_decode($existingRole['role_permissions'], true) ?? [];
        ...
        $isAccessChanged = array_diff($newPermissions, $oldPermissions) || array_diff($oldPermissions, $newPermissions);
        ...
        $updateData = [
            'updated_at' => date('Y-m-d H:i:s'),
            'role_permissions' => json_encode($newPermissions)
        ];
