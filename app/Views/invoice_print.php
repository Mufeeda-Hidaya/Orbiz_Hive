...
foreach ($items as $index => $item):
    ...
    // Calculate line total with 6 decimals
    $lineTotal = bcmul($quantity, $price, 6);

    // Split KD and Fils for item price (1 KD = 1000 Fils)
    $kd = floor($price);
    $fils = str_pad(bcmul(bcsub($price, (string)$kd, 6), '1000000', 0), 6, '0', STR_PAD_LEFT);


    // Split KD and Fils for line total
    $lineKd = floor($lineTotal);
    $lineFils = str_pad(bcmul(bcsub($lineTotal, (string)$lineKd, 6), '1000000', 0), 6, '0', STR_PAD_LEFT);
...
<tfoot class="tfoot">
    ...
    <tr>
        ...
        <td colspan="2" style="text-align: right;">
            <?= number_format($subtotal, 6) ?> KWD
        </td>
    </tr>
    <tr>
        <td colspan="5" style="text-align: right; font-weight: bold;">Discount</td>
        <td colspan="2" style="text-align: right;">
            <?= number_format($effectiveDiscount, 6) ?> KWD
        </td>
    </tr>
    <?php endif; ?>
    <tr>
        ...
        <td colspan="2" style="text-align: right;" id="total-amount">
            <?= number_format($grandTotal, 6) ?> KWD
        </td>
    </tr>
</tfoot>
        ...
        <div style="display: flex; flex-direction: column; align-items: flex-end; margin-top: 10px;">
          <div class="partial-row" id="paidAmountRow"
            ...
            <div class="value" id="paidAmountValue"><?= number_format($paidAmount, 6) ?></div>
          </div>
          <div class="partial-row" id="balanceAmountRow"
            style="display: <?= ($status === 'partial paid' && $paidAmount > 0) ? 'flex' : 'none' ?>;">
            <div class="partial">Balance</div>
            <div class="value" id="balanceAmountValue"><?= number_format($balanceAmount, 6) ?></div>
          </div>
        </div>
      ...
      <div class="modal-content shadow-lg border-0 rounded-4 p-4">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="number" id="markPaidAmount" class="form-control form-control-lg border-success" step="0.000001" readonly>
...
<script>
  ...
  const grandTotal = <?= json_encode(number_format($grandTotal, 6, '.', '')) ?>;
      ...
      .then(data => {
        if (data.success) {
          ...
          if (paidRow && paidVal) {
            ...
            paidVal.innerText = parseFloat(data.paid_amount).toFixed(6);
          }

          if (balanceRow && balanceVal) {
            if (parseFloat(data.balance_amount) > 0) {
              balanceRow.style.display = 'flex';
              balanceVal.innerText = parseFloat(data.balance_amount).toFixed(6);
            } else {
...
// document.getElementById('markPaidAmount').value = balanceAmount.toFixed(3);
 document.getElementById('markPaidAmount').value = balance.toFixed(6);
...
$('#confirmMarkPaid').on('click', function () {
  ...
  const amount = parseFloat($('#markPaidAmount').val()).toFixed(6);
