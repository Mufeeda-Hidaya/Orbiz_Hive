...
<div class="modal fade" id="confirmBlockModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" id="confirm-block-btn" class="btn btn-danger">Yes</button>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.toggle-block', function () {
            ...
            // Store the clicked element so we can update its color after AJAX
            blockIcon = $(this).find('i');
        });
        ...
        $('#confirm-block-btn').click(function () {
            ...
            $.post("<?= base_url('customer/toggleBlock') ?>", { id: blockId }, function (res) {
                if (res.status === 'success') {
                    ...
                    // Change icon color based on new_status
                    if (res.new_status == 3) {
                        blockIcon.css('color', 'red'); // blocked
                    } else {
                        blockIcon.css('color', 'orange'); // active/unblocked
                    }
